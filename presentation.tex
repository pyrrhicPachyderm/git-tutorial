...
\begin{frame}{Configuring R}
	...
	\note[item]{
		...
		\item
		If you are running Windows, first you need to find which version of R you have.
		Go to the relevant directory (as seen on the slides) in your file manager to see which version you have.
		Then, run the first line, changing the version number as necessary.
		Be careful to type all the special characters correctly, including the single and double quotes.
		Close your terminal and open it again, then the last line should work.
	}
\end{frame}
...
\begin{frame}{Adding Changes}
	...
	\note[item]{
		...
		We can also \texttt{git reset -p} to interactively unstage things.
...
\begin{frame}{Diffs}
	\begin{itemize}
		\item \texttt{git diff}
		\item \texttt{git diff HEAD\textasciitilde 1}
		\item \texttt{git diff HEAD\textasciitilde 1 HEAD}
		\item \texttt{git diff HEAD\textasciitilde 1 HEAD -- script.R}
	\end{itemize}
